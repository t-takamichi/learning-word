...
### 3.4. `FlashCard.tsx` (フラッシュカード描画 ＆ 片方訳語耐性)
- [x] **[完了]** 日本語訳が空文字（`""`）でベトナム語訳のみの単語でも、クラッシュせず正常に表裏が表示されることの検証
- [x] **[完了]** ベトナム語訳が空文字（`""`）で日本語訳のみの単語でも、クラッシュせず正常に表裏が表示されることの検証
...
npx vitest run
```
すべての単体テスト（12ファイル、29テストケース）が 100% Green (PASS) しています。
