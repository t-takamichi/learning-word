...
*   **P3: モバイル片手操作・共有デバイス環境での使いやすさを守る** (Mobile & Shared-Device First)
    *   隙間時間に片手で操作するモバイルユーザーのために親指到達範囲（FAB）やスワイプジェスチャーを設計し、同時にPCやタブレットを共有するファミリー/教育環境のためにキーボード手入力を排除した「アバター選択ログイン」を設計します。また、iOS Safariの自動再生制限などのデバイス固有の制約を完全に解決します。
