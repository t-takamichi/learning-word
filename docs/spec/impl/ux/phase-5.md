...
|----|------------------|------|------|----|---------|-----------|
| T1 | `organisms/FlashCard/*` | 移行(刷新) | 既存を移設。面 `--surface`/角丸 `--radius-card`/影 `--shadow-soft`。英単語 `--ink-900`＋任意で `--berry-200` 下線。裏返し Y 軸フリップ(180ms)・`prefers-reduced-motion` でクロスフェード。「こたえを見る」ボタン | Organism | 目視＋フリップ／reduced-motion | visual-design §5.1, react.md |
